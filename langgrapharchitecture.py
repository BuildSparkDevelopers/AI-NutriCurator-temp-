...
tool calling을 위한 래핑 필요 - 챗지피티 대화 참고
"""
...
model = AutoModelForCausalLM.from_pretrained(
    MODEL_NAME,
    torch_dtype=torch.bfloat16,  #플롯 앞에 b추가
    device_map="auto",
    trust_remote_code=True
)
...
#orchagnet = RouterLogic(router_llm) - llm 사용의 합리성 검토 후 비사용
...
# main.py 또는 graph.py 상단
from langchain_openai import ChatOpenAI

# 라우터 로직에서는 llm 사용 안하기로
#router_llm = ChatOpenAI(
#    model="gpt-4o",
#    temperature=0,   # 판단의 일관성을 위해 0으로 설정
#    api_key="sk-..." # (환경변수로 관리)
#)
...
#### 섭스티튜션 에이전트 : Tool calling

조건식으로 분기하는거랑
아무거나 치면 네가 뭐가 필요한지 판단해서 calling해서 써 가 다름
오케스트레이션이 알아서 툴 호출해서 쓰게하려면 tool calling

tool calling 기능을 지원하는 모델이 있음
아닌 경우 ollama 래핑 사용가능
gpt api 쓰면 정확하고 쉽고 빠르게 잘 됨
local model (ex) qwen -

입력된 정보에 대해 가치판단을 하거나 예측을 해서 동작해야함

tool calling 컨셉:
llm이 하기 어려운 동작들 -명확한 수식을 통한 계산이라던가 외부 호출이라던가 그거를 더 잘하는 도구를 호출하는것
"""
...
# workflow.compile()을 마친 app 객체를 사용합니다.
try:
    display(Image(app.get_graph().draw_mermaid_png()))
except Exception:
    # 환경에 따라 추가 라이브러리(pyppeteer 등)가 필요할 수 있습니다.
    print("그래프를 시각화할 수 없습니다.")
